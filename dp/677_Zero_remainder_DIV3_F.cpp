...
int dp[71][71][71][71];
void solve(){
    int n,m,K; cin>>n>>m>>K; int arr[n][m];
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++){
            cin>>arr[i][j];
        }
        ...
        for(int j=0;j<m;j++){
            for(int k=0;k<(m)/2 + 1;k++){
                for(int l=0;l<K;l++){
                    ...
                    if (k + 1 <= m / 2) {
                        if (i != ni) {
                            dp[ni][nj][0][(l + arr[i][j]) % K] = max(dp[ni][nj][0][(l + arr[i][j]) % K], dp[i][j][k][l] + arr[i][j]);
                        } else {
                            dp[ni][nj][k + 1][(l + arr[i][j]) % K] = max(dp[ni][nj][k + 1][(l + arr[i][j]) % K], dp[i][j][k][l] + arr[i][j]);
                        }
                    }
                }
            }
